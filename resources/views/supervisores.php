  ...
  <div id="app">
    <v-app>
        ...
        <v-simple-table class="mt-5">
            <template v-slot:default>
                ...
                <tbody>
                    ...
                    <td class="text-center">{{ Supervisor.supervisors_nombre }}</td>
               
                    <td class="text-center">
                        <v-btn fab dark color="#00BCD4" small @click="formEditar(Supervisor.id,Supervisor.supervisors_nombre)"><v-icon>mdi-pencil</v-icon></v-btn>
                        <v-btn fab dark color="#E53935" small @click="borrar(Supervisor.id)"><v-icon>mdi-delete</v-icon></v-btn>
                    </td>
                    </tr>
                </tbody>
            </template>
      ...
      <v-dialog v-model="dialog" max-width="500">        
        <v-card>
          <v-card-title class="blue darken-2 white--text">Supervisor</v-card-title>    
          <v-card-text> 
           <v-form>        
              <v-container>
                <v-row>
                  <input v-model="Supervisor.id" id="id" hidden></input>
                  <v-col cols="12" md="12">
                    <v-text-field v-model="Supervisor.supervisors_nombre" :rules="[v => !!v || 'El campo nombre es obligatorio!']" id="nombre" label="nombre" required>{{Supervisor.supervisors_nombre}}</v-text-field>
                  </v-col>
                </v-row>
              </v-container>            
          </v-card-text>
...
new Vue({
  ...
  vuetify: new Vuetify(),
   ...
   methods:{          
        ...
        crear:function(){
            let parametros = {nombre:this.Supervisor.supervisors_nombre};                
            axios.post(url, parametros)
            .then(response =>{
              this.mostrar();
            });     
            this.Supervisor.supervisors_nombre="";
          
        },                        
        editar: function(){
        let parametros = {nombre:this.Supervisor.supervisors_nombre, id:this.Supervisor.id};                
        ...
        formEditar:function(id, nombre){
          ...
          this.Supervisor.supervisors_nombre = nombre;          
          this.dialog=true;                            
